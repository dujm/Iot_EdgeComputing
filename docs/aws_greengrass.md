...
# Delete the function and subscription from the group
 i) From the Lambdas page => choose the ellipsis (…) => Remove function

 ii) From the Subscriptions page => choose the ellipsis (…) => choose Delete
 
 iii) The function and subscription are removed from the core during the next group deployment
```
<br>

### 4.4 Module 3 (Part 2): Lambda Functions on AWS IoT Greengrass
 * configure a Lambda function 
 * deploy it to your AWS IoT Greengrass core device
...
# https://docs.aws.amazon.com/greengrass/latest/developerguide/on-demand.html?shortFooter=true
```
...
--- 
### Errors
1. **Error 1**: AWS Greengrass detected insecure OS configuration: No hardlink/softlink protection enabled.
